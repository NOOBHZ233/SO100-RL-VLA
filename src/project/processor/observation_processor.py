...
from project.configs.types import PipelineFeatureType, PolicyFeature
from project.utils.constans import OBS_ENV_STATE, OBS_IMAGE, OBS_IMAGES, OBS_STATE, OBS_STR
...
class VanillaObservationProcessorStep(ObservationProcessorStep):
    ...
    **State Processing:**
    ...
    def _process_observation(self, observation):
        ...
        # 处理 agent_pos（标准情况）或 joint_pose + gripper_pose（gym_so100 情况）
        ...
        elif "joint_pose" in processed_obs:
            # 处理 gym_so100 环境的观察数据：合并 joint_pose 和 gripper_pose
            joint_pose_np = processed_obs.pop("joint_pose")
            if "gripper_pose" in processed_obs:
                gripper_pose_np = processed_obs.pop("gripper_pose")
                # 合并关节位置和夹爪位置
                agent_pos_np = np.concatenate([joint_pose_np, gripper_pose_np])
            else:
                agent_pos_np = joint_pose_np
