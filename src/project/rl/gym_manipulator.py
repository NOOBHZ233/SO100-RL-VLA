...
from project.cameras import opencv 
from project.configs import parser
from project.datasets.lerobot_dataset import LeRobotDataset
from project.envs.configs import HILSerlRobotEnvConfig
from project.model.kinematics import RobotKinematics
...
from project.processor.converters import identity_transition
from project.robots import (  # noqa: F401
    RobotConfig,
    make_robot_from_config,
    so100_follower,
)
from project.robots.robot import Robot
from project.robots.so100_follower.robot_kinematic_processor import (
    EEBoundsAndSafety,
    EEReferenceAndDelta,
...
def make_robot_env(cfg: HILSerlRobotEnvConfig) -> tuple[gym.Env, Any]:
    ...
    if cfg.name == "gym_so100" :
        assert cfg.robot is None and cfg.teleop is None, "GymHIL environment does not support robot or teleop"
        import sys
        sys.path.insert(0, "/home/zcw/SO100-RL/RL/mycode")
        ...
        env = gym.make(
            f"gym_so100/{cfg.task}",
            # 不指定render_mode,让环境使用默认配置(human模式用于PassiveViewerWrapper)
        )
...
def make_processors(
    ...
    # GymSO100 simulation environment
    if cfg.name == "gym_so100":
        action_pipeline_steps = [
            InterventionActionProcessorStep(terminate_on_success=terminate_on_success),
            Torch2NumpyActionProcessorStep(),
